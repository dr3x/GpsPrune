...
public class GpsLoader extends GenericFunction implements Runnable
{
	...
	public void run()
	{
		...
		if (_waypointCheckbox.isSelected() || _trackCheckbox.isSelected())
		{
			...
			{
				_pruneApp.showErrorMessageNoLookup(getNameKey(), e.getMessage());
				_cancelled = true;
			}
		}
		setupProgressBar(true);
	...
	private void callGpsBabel() throws Exception
	{
			...
			if (process.exitValue() == 0)
			{
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						...
						_pruneApp.loadDataFiles(fileList);
					}
				});
			}
		...
		else
		{
			...
			try
			{
				XmlFileLoader xmlLoader = new XmlFileLoader(_pruneApp);
			...
			SourceInfo sourceInfo = new SourceInfo(_deviceField.getText(), SourceInfo.FILE_TYPE.GPSBABEL);
			FileInfo fileInfo = handler.getFileInfo();
			fileInfo.addSource(sourceInfo);
			_app.onFileLoaded(fileInfo);
			_pruneApp.onFileLoaded(fileInfo);
		}
	}
