...
public class XmlFileLoader extends DefaultHandler implements Runnable
{
	...
	private FileLoadedInterface _fileLoaded = null;
	 ...
	 * @param inFileLoadedInterface Object to inform of track load (usually equals inApp)
	 */
	public XmlFileLoader(App inApp, FileLoadedInterface inFileLoaded)
	{
		_app = inApp;
		_fileLoaded = inFileLoaded;
	}
	...
	public void run()
	{
		...
		try
		{
			...
			else
			{
				...
				FileInfo fileInfo = _handler.getFileInfo();
				fileInfo.addSource(sourceInfo);
				_fileLoaded.onFileLoaded(fileInfo);
			}
		}
		catch (Exception e)
		{
		...
		Attributes attributes) throws SAXException
	{
		...
		{
			if (qName.equals("kml")) {
				if( _app instanceof DrawApp ) {
					_handler = new KmlDrawingHandler(((DrawApp)_app).getDrawing());
				} else {
			...
			} else if (_unknownType == null && !qName.equals("")) {_handler.startElement(uri, localName, qName, attributes);
				_unknownType = qName;
			}
		}
		else
