...
public class KmlDraw {
	...
	private static void launch(ArrayList<File> inDataFiles)
	{
		...
		// make menu
		DrawMenuManager menuManager = new DrawMenuManager(APP);
		...
		APP.getToolbox().addTools(toolPanel);
		
		MapCanvas mapDisp = new DrawingMapCanvas(APP);
		...
		UpdateMessageBroker.informSubscribers("Draw");

		frame.getContentPane().setLayout(new BorderLayout());
		frame.getContentPane().add(toolPanel, BorderLayout.NORTH);
		frame.getContentPane().add(mapDisp, BorderLayout.CENTER);
		...
		// set icon
		try {
			frame.setIconImage(IconManager.getImageIcon(IconManager.DRAW_WINDOW_ICON).getImage());
		}
		...
		if (inDataFiles == null || inDataFiles.size() == 0) {
			GotoManager.getInstance().gotoStartView();
		}
	}
}
