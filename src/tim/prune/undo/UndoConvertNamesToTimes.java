...
public class UndoConvertNamesToTimes implements UndoOperation
{
	...
	public void performRedo(App app) throws UndoException {
		...
		for (int i=_startIndex; i<=_endIndex; i++)
		{
			if (_waypointNames[i-_startIndex] != null)
			{
				Timestamp tstamp = new Timestamp(_waypointNames[i-_startIndex]);
				if (tstamp.isValid()) {
					DataPoint point = track.getPoint(i);
					// timestamp could be parsed!
					point.setFieldValue(Field.TIMESTAMP, _waypointNames[i-_startIndex], true);
					// set waypoint name to nothing (track point)
					point.setFieldValue(Field.WAYPT_NAME, null, true);
				}
			}
		}
	}
