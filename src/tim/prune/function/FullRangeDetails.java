...
public class FullRangeDetails extends GenericFunction
{
	...
	{
		Selection selection = _pruneApp.getTrackInfo().getSelection();
		// Number of segments
		...
		// Gradient
		Altitude firstAlt = _pruneApp.getTrackInfo().getTrack().getPoint(selection.getStart()).getAltitude();
		Altitude lastAlt = _pruneApp.getTrackInfo().getTrack().getPoint(selection.getEnd()).getAltitude();
		...
		// Maximum speed
		SpeedData speeds = new SpeedData(_pruneApp.getTrackInfo().getTrack());
		speeds.init();
		double maxSpeed = 0.0;
