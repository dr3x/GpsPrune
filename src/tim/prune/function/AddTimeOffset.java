...
public class AddTimeOffset extends GenericFunction
{
	...
	public void begin()
	{
		...
		if (!_pruneApp.getTrackInfo().getTrack().hasData(Field.TIMESTAMP, selStart, selEnd))
		{
			_pruneApp.showErrorMessage(getNameKey(), "dialog.addtimeoffset.notimestamps");
			return;
		}
		// Make dialog window
		if (_dialog == null)
	...
	private void finish()
	{
		...
		if (offsetSecs != 0L)
		{
			...
			_pruneApp.finishAddTimeOffset(offsetSecs);
			_dialog.dispose();
		}
	}
}
