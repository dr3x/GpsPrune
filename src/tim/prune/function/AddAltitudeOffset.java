...
public class AddAltitudeOffset extends GenericFunction
{
	...
	public void begin()
	{
		...
		if (!_pruneApp.getTrackInfo().getTrack().hasData(Field.ALTITUDE, selStart, selEnd))
		{
			_pruneApp.showErrorMessage(getNameKey(), "dialog.addaltitude.noaltitudes");
			return;
		}
		// Make dialog window
		if (_dialog == null)
	...
	private void setLabelText(int inStart, int inEnd)
	{
		...
		{
			Altitude alt = _pruneApp.getTrackInfo().getTrack().getPoint(i).getAltitude();
			if (alt != null) {
	...
	private void finish()
	{
		...
		_pruneApp.finishAddAltitudeOffset(_editField.getText(), _altFormat);
		_dialog.dispose();
	}
}
