...
public class AboutScreen extends GenericFunction
{
	...
	private Component makeContents()
	{
		...
		String versionId = version.getProperty("ci.buildId");
		if( versionId == null || versionId.trim().length() == 0 ) {
			versionId = version.getProperty("rcs.buildId","???");
		}
		...
		JLabel titleLabel = new JLabel(_app instanceof DrawApp ? "Draw" : "Prune");
		titleLabel.setFont(new Font("SansSerif", Font.BOLD, 24));
		titleLabel.setAlignmentX(JLabel.CENTER_ALIGNMENT);
		aboutPanel.add(titleLabel);
		JLabel versionLabel = new JLabel(I18nManager.getText("dialog.about.version") + ": " + versionNumber);
		versionLabel.setAlignmentX(JLabel.CENTER_ALIGNMENT);
		aboutPanel.add(versionLabel);
		JLabel buildLabel = new JLabel(I18nManager.getText("dialog.about.build") + ": " + versionId);
		...
		final int NUM_INSTALL_CHECKS = 2;
		...
		addToGridBagPanel(creditsPanel, gridBag, constraints,
			...
			0, 7);
		addToGridBagPanel(creditsPanel, gridBag, constraints,
			new JLabel("Openstreetmap, Google Earth, Gpsbabel, Gnuplot"),
			1, 7);
		addToGridBagPanel(creditsPanel, gridBag, constraints,
	...
	private void checkInstalls()
	{
		...
		final int[] tools = {ExternalTools.TOOL_GPSBABEL, ExternalTools.TOOL_GNUPLOT};
		for (int i=0; i<tools.length; i++) {
			_installedLabels[i].setText(ExternalTools.isToolInstalled(tools[i])?yesText:noText);
		}
	}
	
	private Properties readVersion() {
		Properties properties = new Properties();
		try {
			properties.load(AboutScreen.class.getResourceAsStream("/version.properties"));
		} catch ( Exception e ) {
			Logger.getLogger(AboutScreen.class.getName()).log(Level.SEVERE, "Failed to read version info", e);
		}
		return properties;
	}
}
