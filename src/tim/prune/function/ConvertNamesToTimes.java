...
public class ConvertNamesToTimes extends GenericFunction
{
	...
	public void begin()
	{
		...
		{
			_pruneApp.showErrorMessage(getNameKey(), "error.convertnamestotimes.nonames");
			return;
		}
		UndoConvertNamesToTimes undo = new UndoConvertNamesToTimes(_pruneApp.getTrackInfo());
		int numConverted = 0;
		...
		if (numConverted > 0)
		{
			_pruneApp.getTrackInfo().getTrack().requestRescale();
			UpdateMessageBroker.informSubscribers(DataSubscriber.DATA_EDITED);
			_pruneApp.completeFunction(undo, I18nManager.getText("confirm.convertnamestotimes"));
		}
	}
