...
public class DeleteFieldValues extends GenericFunction
{
	...
	public void begin()
	{
		...
		if (_listModel.getSize() < 1) {
			_pruneApp.showErrorMessage(getNameKey(), "dialog.deletefieldvalues.nofields");
		}
		else {
	...
	private void initDialog()
	{
		...
		int selStart = _pruneApp.getTrackInfo().getSelection().getStart();
		int selEnd = _pruneApp.getTrackInfo().getSelection().getEnd();
		// Loop over fields in track
		final Track track = _pruneApp.getTrackInfo().getTrack();
	...
	private void finish()
	{
		...
		if (field != null)
		{
			UndoDeleteFieldValues undo = new UndoDeleteFieldValues(_pruneApp.getTrackInfo(), field);
			final int selStart = _pruneApp.getTrackInfo().getSelection().getStart();
			final int selEnd = _pruneApp.getTrackInfo().getSelection().getEnd();
			final Track track = _pruneApp.getTrackInfo().getTrack();
			for (int i=selStart; i<= selEnd; i++) {
				track.getPoint(i).setFieldValue(field, null, false);
			}
			_dialog.dispose();
			_pruneApp.getTrackInfo().getSelection().markInvalid();
			UpdateMessageBroker.informSubscribers(DataSubscriber.DATA_EDITED);
			_pruneApp.completeFunction(undo, I18nManager.getText("confirm.deletefieldvalues"));
		}
	}
}
