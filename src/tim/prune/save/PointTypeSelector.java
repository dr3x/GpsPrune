...
{
	private static final long serialVersionUID = 3675779252473406054L;
	/** Array of checkboxes */
	...
	private void createComponents()
	{
		...
		for (int i=0; i<2; i++)
		{
			...
			_gridPanel.add(_checkboxes[i]);
		}
		add(_gridPanel, BorderLayout.CENTER);
		_checkboxes[2] = new JCheckBox(I18nManager.getText("dialog.pointtype.selection"));
		add(_checkboxes[2], BorderLayout.SOUTH);
	}
	...
	public void init(TrackInfo inTrackInfo)
	{
		...
		final boolean[] showFlags = {true, true};
		...
		_checkboxes[2].setEnabled(inTrackInfo.getSelection().hasRangeSelected());
		_checkboxes[2].setSelected(false);
	}
