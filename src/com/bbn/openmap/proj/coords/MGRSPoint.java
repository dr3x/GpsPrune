...
public class MGRSPoint extends ZonedUTMPoint {
    ...
    public void setMGRS(String mgrsString) {
        try {
            decode(mgrsString.toUpperCase());
            // Make sure mgrs and mgrsPretty are set and normalized.
            resolve();
    ...
    /**
     * Get the MGRS string value - the honkin' coordinate value.
     * This one inserts spaces to make it look nicer.
     */
    public String getPrettyMGRS() {
        if (mgrsPretty == null) {
            resolve();
        }
        return mgrsPretty;
    ...
    public String getZone() {
        return zone_number + "" + (char) zone_letter;
    }
     ...
     * Converts a LatLonPoint to a MGRS Point, assuming the WGS_84 ellipsoid.
     * 
     * @param zoneNumber Zone to force point into.
     * @return MGRSPoint, or null if something bad happened.
     */
    public static MGRSPoint LLtoMGRS(LatLonPoint llpoint, int zoneNumber) {
        return LLtoMGRS(llpoint, Ellipsoid.WGS_84, new MGRSPoint(), zoneNumber);
    }
     ...
     * @param zoneNumber Zone to force this point into.
    ...
    public static MGRSPoint LLtoMGRS(LatLonPoint llp, Ellipsoid ellip,
        ...
        int zoneNumber = getZoneNumber(llp.getLatitude(), llp.getLongitude());
        // Calling LLtoUTM here results in N/S zone letters! wrong!
        mgrsp = (MGRSPoint) LLtoMGRS(llp, ellip, mgrsp, zoneNumber);
     ...
     */
    public static MGRSPoint LLtoMGRS(LatLonPoint llp, Ellipsoid ellip,
    		MGRSPoint mgrsp, int zoneNumber) {
        if (mgrsp == null || !(mgrsp instanceof MGRSPoint)) {
            mgrsp = new MGRSPoint();
        }

        // Calling LLtoUTM here results in N/S zone letters! wrong!
        mgrsp = (MGRSPoint) LLtoUTM(llp, ellip, mgrsp, zoneNumber, (llp.getLatitude() >= 0f));
        // Need to add this to set the right letter for the latitude.
        mgrsp.zone_letter = mgrsp.getLetterDesignator(llp.getLatitude());
        mgrsp.resolve();
        return mgrsp;
    ...
    protected void decode(String mgrsString) throws NumberFormatException {
        ...
        // Strip spaces to allow entry in "pretty" format.
        // Should probably be smarter about this (i.e. space context should matter).
        StringBuffer wsb = new StringBuffer(mgrsString.length());
        for (int x = 0; x < mgrsString.length(); x++) {
        	if (!Character.isWhitespace(mgrsString.charAt(x)))
        		wsb.append(mgrsString.charAt(x));
        }
        mgrsString = wsb.toString();
    ...
    public void resolve() {
        if (zone_letter == 'Z') {
            mgrs = mgrsPretty = "Latitude limit exceeded";
        } else {
            ...
            try {
                ...
                mgrsPretty = sb.toString();
                // Remove spaces for the non-pretty MGRS
                StringBuffer wsb = new StringBuffer(mgrsPretty.length());
                for (int x = 0; x < mgrsPretty.length(); x++) {
                	if (!Character.isWhitespace(mgrsPretty.charAt(x)))
                		wsb.append(mgrsPretty.charAt(x));
                }
                mgrs = wsb.toString();
    ...
    public String get100kID() {
    	return get100kID(easting, northing, zone_number);
    }
     ...
     * zone.
     * 
     * ported from Geotrans' c Lattitude_Band_Value structure table. zoneLetter :
     * MGRS zone (input)
     */
